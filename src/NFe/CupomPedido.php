...
class CupomPedido extends Common
{
    protected $papel;
    ...
    public function monta(
        ...
        $hLinha = $this->hLinha;
        $tamPapelVert = 70 + 16 + 12 + (($qtdItens - 1) * $hMaxLinha) + ($qtdPgto * $hLinha);
        ...
        $hcabecalho = 25;//para cabeçalho (dados emitente mais logomarca)  (FIXO)
    ...
    {
        $config = ConfigNota::
        where('empresa_id', $this->pedido->empresa_id)
        ->first();
    ...
    protected function cabecalhoSecundarioDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        $w = ($maxW*1);
        $hBox1 = 7;
        $texto = "Comanda " . $this->pedido->comanda;
        if($this->pedido->cliente){
            $texto .= "\n" . $this->pedido->cliente->razao_social;
        }
        $aFont = array('font'=>$this->fontePadrao, 'size'=>12, 'style'=>'B');
        $this->pdf->textBox($x, $y+3, $w, $hBox1, $texto, $aFont, 'C', 'C', 0, '', false);
        $hBox2 = 4;

        if($this->pedido->nome != ''){
            $hBox1 = 20;
            $texto = "Cliete: ". $this->pedido->nome . " " . $this->pedido->telefone;
            $aFont = array('font'=>$this->fontePadrao, 'size'=>12, 'style'=>'B');
    ...
    protected function produtosDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        if ($qtdItens > 0) {
            foreach ($this->pedido->itens as $p) {
                ...
                $vUnCom     = number_format($p->valor/$p->quantidade, 2, ",", ".");

                $valorProduto = $p->valor;
                // foreach($p->itensAdicionais as $a){
                //     $valorProduto += $p->quantidade * $a->adicional->valor;
                // }
    ...
    protected function totalDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        $aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
        $this->pdf->textBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
        $texto = $this->pedido->rua;
        if($this->pedido->cliente){
            $texto = $this->pedido->cliente->rua;
        }
        $aFont = ['font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B'];
        $this->pdf->textBox($xValor-20, $yTotalFinal, $wColDir+20, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
        $yTotalFinal = $y + ($hLinha*7);

        $texto = "Número";
        $aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
        $this->pdf->textBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
        $texto = $this->pedido->numero;
        if($this->pedido->cliente){
            $texto = $this->pedido->cliente->numero;
        }
        $aFont = ['font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B'];
        $this->pdf->textBox($xValor-10, $yTotalFinal, $wColDir+10, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
        ...
        if($this->pedido->cliente){
            $texto = $this->pedido->cliente->bairro;
        }
        $aFont = ['font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B'];
        $this->pdf->textBox($xValor-15, $yTotalFinal, $wColDir+15, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
        ...
        if($this->pedido->cliente){
            $texto = $this->pedido->cliente->referencia;
        }
        $aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
        $this->pdf->textBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
        $yTotalFinal = $y + ($hLinha*10);

        if($this->pedido->cliente){
            $texto = "Complemento";
            $aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
            $this->pdf->textBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
            $texto = $this->pedido->cliente->complemento;
        }
        $aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
        $this->pdf->textBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
        $yTotalFinal = $y + ($hLinha*11);
